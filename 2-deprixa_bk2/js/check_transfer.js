...
//* inicio y configuración de data tables más información en: https://datatables.net/   *//
    ...
    columns: [ // columnas que seran mostradas este debe ser igual a la cantidad de <theads> de la tabla        
        ...
        { data: 'product_name' },
        { data: 'ship_qty' },
        { data: 'c_container_number' },
        { data: 't_container_number' },
    ]
    });
...
//function para guardar usuario en la BD
    ...
    $(document).on("click", '#transfer_submit', function (event) {
        var err = '';
        if( $('#f_location').val() == '') {
           err += "Please select From Location. <br>"; 
        }
        if( $('#t_location').val() == '') {
            err += "Please select To Location.<br>"; 
        }
        if( $('#container_number').val() == '') {
            err += "Please select Container Number.<br>"; 
        }
        if( $('#order_id').val() == '') {
            err += "Please enter Order Id.<br>"; 
        }
        if( $('#line_item').val() == '') {
            err += "Please enter Line Item.<br>"; 
        }
        ...
        var datos = $('#formularioNuevo').serialize();
        ...
        $.ajax({
                        ...
                        success: function (data) {
                            if (data.msg == 'ok') {
                                ...
                                $('#nuevo').modal('toggle');
                                notificar(" Transfer has been done successfully.", "info");
                                treload.ajax.reload();
                            } else {
                                notificar("Error could not transfer data.", "error");
                            }
                        }
                    });
    });
...
// eventos de keypress formulario nuevo usuario sirve para mostrar error en campos de formulario si estos estan vacios método utilizando clases
    ...
    $('.pricefld').keypress(function (event) {
        if ($('.pricefld').val().length > 1) { // cantidad de caracteres mayor a 1
            //quitamos la clase error
            $('#priceval').removeClass('has-error');
        } else {
            //agregamos la clase error
            $('#priceval').addClass('has-error');
        }
    });
...
// función para recuperar datos del usuario a editar
    $(document).on("click", '.editar', function (event) {
        ...
        $.ajax({// configuración de ajax
            ...
            $('#formularioEditar input[name="price"]').val(data[0]['price']);
