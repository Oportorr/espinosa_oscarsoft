...
function addCons(){
	...
	$grand_total = $_POST['grand_total'];
        ...
        $last_invice_no = mysql_query("SELECT invoice_number FROM `courier` ORDER BY `invoice_number` DESC LIMIT 1");
        $lastiniceno = mysql_fetch_array($last_invice_no);
        $lastiniceno = $lastiniceno['invoice_number'];
        #last number of containner number
        $container_last_id = substr($container_number, -1);
        $invoice_number = '';
        if($container_last_id > 0){
          $last_invice_no = mysql_query("SELECT invoice_number FROM `courier` where container_number=".$container_number." ORDER BY `invoice_number` DESC LIMIT 1 ");
          
          $lastiniceno = mysql_fetch_array($last_invice_no);
          
          $lastiniceno = $lastiniceno['invoice_number'];
          if($lastiniceno != ''){
              $invoice_number = $lastiniceno + 1; 
          }else{
             $invoice_number = $container_last_id. '01'; 
          }
        }
        
        $sql = "INSERT INTO courier (cons_no, ship_name, phone, s_add, s_city, s_state, s_country, s_zipcode, cc, rev_name, r_phone, r_add, r_city, r_state ,r_country, cc_r, email, type, weight, variable, shipping_subtotal, invice_no, invoice_number, qty, book_mode, freight, declarate, mode, pick_date, payment_due, schedule, pick_time, status, comments, officename, user, book_date, container_id, container_number, office_origin)
			VALUES('$ConsignmentNo', '$Shippername','$Shipperphone', '$Shipperaddress', '$Shippercity', '$Shipperstate', '$Shippercountry', '$Shipperzip_code', '$Shippercc', '$Receivername','$Receiverphone','$Receiveraddress', '$Receivercity', '$Receiverstate', '$Receivercountry', '$Receivercc_r', '$Receiveremail', '$Shiptype',$Weight , '$variable', $grand_total, '$Invoiceno', $invoice_number, $Qnty, '$Bookingmode', '$Totalfreight',  '$Totaldeclarate', '$Mode', '$Packupdate', '$Payment_Due', '$Schedule', '$Pickuptime', '$status', '$Comments', '$officename', '$user', curdate(), '$container_id1', '$container_number', '$office_origin')";	
	//	echo $sql;  
	dbQuery($sql);
