...
function addCons(){
	...
	$grand_total = $_POST['grand_total'];
        ...
        $last_invice_no = mysql_query("SELECT invice_no FROM `courier` ORDER BY `invice_no` DESC LIMIT 1");
        $lastiniceno = mysql_fetch_array($last_invice_no);
        $lastiniceno = $lastiniceno['invice_no'];
        #last number of containner number
        $container_last_id = substr($container_number, -1);
        $invoice_number = '';
        if($container_last_id > 0){
          $last_invice_no = mysql_query("SELECT invice_no FROM `courier` where container_number=".$container_number." ORDER BY `cid` DESC LIMIT 1 ");
