...
$rs = " SELECT c.cid,c.invoice_number, c.modified_by,c.phone,c.r_phone,c.modified_date,c.officename,c.book_date,c.ship_name,c.s_zipcode,c.comments,c.s_add,c.paid_amount,c.shipping_subtotal,c.paid_amount, ct.city_name , st.state_name , co.country_name FROM `courier` c
...
$html .='<td align="center" style="font-family: serif; font-size: 18px;font-weight: bold;">Order Id</td>';
...
while ($row = mysql_fetch_array($getresult)) {
    ...
    $html .='<tr><td height="80px" width="50px" >' . $row['invoice_number'] . '</td>';
