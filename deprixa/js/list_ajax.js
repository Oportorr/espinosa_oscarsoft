...
var treload = $('#tabla-usuarios').DataTable({
	...
	ajax: { // configuración ajax
            "url": "settings/container/list_container_ajax.php",
            "type": "GET",
			"dataSrc": ""
    },
    ...
    columns: [ // columnas que seran mostradas este debe ser igual a la cantidad de <theads> de la tabla
        ...
        { data: 'container_number' }, // recuperamos y mostramos 
...
function(){ // esta función se ejecuta cuando confirmamos  por si
	...
	$.ajax ({ // configuración de ajax
				type: "POST",
				url: "settings/container/eliminar.php",
				data: { "id" : id },
				dataType:"json"
				}).done(function(data){
					if(data.msg=='ok'){
...
function(){
	...
	$.ajax ({ // configuración de ajax
				type: "POST",
				url: "settings/container/estado.php",
				data: { "id" : id },
				dataType:"json"
				}).done(function(data){
					if(data.msg=='ok'){
...
$(document).on("click", '.editar', function (event) {
			...
			$.ajax ({ // configuración de ajax
            type: "POST",
            url: "settings/container/editar.php",
            data: { "container_id":id },
			dataType: 'json'
        	}).done(function(data){
                    
		         	$('input[name="id"]').val(data[0]['container_id']);
					$('input[name="containernumber"]').val(data[0]['container_number']);
					$('input[name="containerdescription"]').val(data[0]['container_description']);
					$('input[name="container_date"]').val(data[0]['container_date']);
					$('input[name="edit_notes"]').val(data[0]['container_notes']);
					if(data[0].container_status=='1'){ // marcamos o desmarcamos el check box activo según estado del usuario
...
$(document).on("click", '#guardarNuevo', function (event) {
  //  console.log('hiiiii');
   var id=  $('input[name="id"]').val();
                        var container_number= $('input[name="containernumber"]').val();
                         var container_description= $('input[name="containerdescription"]').val();
						 var container_date= $('input[name="container_date"]').val();
                          var container_notes= $('input[name="edit_notes"]').val();
//var datos = $('#formularioNuevo').serialize();	// recuperamos todos los campos del formulario
             
              
			$.ajax ({ // configuraci{on de ajax
            type: "POST",
            url: "settings/container/actualizar.php",
            data: { "container_id":id,"container_number":container_number,"container_description":container_description,"container_date":container_date,"container_notes":container_notes},
			dataType:"json"
        	}).done(function(data){
