// Funciones principales del mÃ³dulo CRUD //
...
//Funcion "notificar" para mostrar notificaiones recibe 2 variables uno el mensaje y el otro el estilo de mensaje
    function notificar(mensaje, tipo) {
        ...
        if (tipo == 'ok') {
            $.notify({// plugin bootstrap-notify mÃ¡s informaciÃ³n en: http://bootstrap-growl.remabledesigns.com/
        ...
        // estilo de mensaje informaciÃ³n "info"
        if (tipo == 'info') {
            $.notify({
        ...
        //* aqui podras seguir agregando mÃ¡s notificaciones perzonalizadas, recuerda revisar la documentaciÃ³n de los plugins. *//

    }

//* inicio y configuraciÃ³n de data tables mÃ¡s informaciÃ³n en: https://datatables.net/   *//
    var treload = $('#tabla-usuarios').DataTable({
        ...
        tableTools: {
            ...
            "aButtons": [// configuraciÃ³n de botones
                {
                    "sExtends": "copy",
        ...
        "oLanguage": {
            "sUrl": "plugins/dataTables/dataTables.spanish.txt"  // idioma espaÃ±ol para data tables
        },
        "columnDefs": [{// insertamos las operaciones en la posiciÃ³n 5 al final de la tabla
                "targets": 6,
...
// aÃ±adir estilos a los otones de exportaciÃ³n
    var tt = new $.fn.dataTable.TableTools(treload); // buscamos la extensiÃ³n table tools
...
// agregar foco al campo de texto nombre en modal nuevo usuario, cuando la ventana modal este visible
    $('#nuevo').on('shown.bs.modal', function (event) {
        $('#formularioNuevo')[0].reset();	// reseamos el formulario para aÃ±adir un nuevo registro
        ...
        $.ajax({// configuraci{on de ajax
            ...
            switch (data.msg) {
                case 'ok':
                    ...
                    $('#nuevo').modal('toggle');
                    // mostramos una notificaciÃ³n satisfactoria
                ...
                case 'nomvacio':
                    // mostramos una notificaciÃ³n de error
                    notificar("Error the Office field is required.", "error");
                    //aÃ±adimos una clase error al campo nombre
                ...
                case 'pasvacio':
                    ...
                    break;
                    //* Aqui puedes aÃ±adir mas mensajes que provengan del servidor *//
                case 'defaultorigin':
                    //notificar("Default origin aready present!","error");
                    $('#default_orgin_msg').html('<p style="color: red;"> Default Office already assigned!</p>');
...
// funcion para guardar editar usuario
    $(document).on("click", '#actualizar', function (event) {
        ...
        $.ajax({// configuraci{on de ajax
            ...
            if(data.msg == 'defaultorigin'){
                //$('#edit_default_orgin_msg').html('<p style="color: red;"> Default Office already assigned!</p>');
                notificar(" Default Office already assigned!", "error");
                return false;
            }
            if (data.msg == 'ok') {
...
// eventos de keypress formulario nuevo usuario sirve para mostrar error en campos de formulario si estos estan vacios mÃ©todo utilizando clases
...
// eventos de keypress formulario editar usuario sirve para mostrar error en campos de formulario si estos estan vacios mÃ©todo utilizando ids
...
//funcion para eliminar usuario
    $(document).on("click", '.eliminar', function (event) {
        ...
        swal({// plugin sweet alert mÃ¡s informaciÃ³n en: http://t4t5.github.io/sweetalert/
            title: "Â¿Are you sure?", // tÃ­tulo del alert
            text: "This office will be completely removed from the database", // texto del alert
            type: "warning", // estilo del alert
            showCancelButton: true, // mostrar botÃ³n de cancelar
            cancelButtonText: "Cancel", // texto alternativo del botÃ³n por defecto cancel
            confirmButtonColor: "#DD6B55", // color del botÃ³n confirmar
            confirmButtonText: "Yes, Delete", // texto del botÃ³n confirmar
            closeOnConfirm: false // cerrar alert al confirmar
        },
                function () { // esta funciÃ³n se ejecuta cuando confirmamos  por si

                    $.ajax({// configuraciÃ³n de ajax
                        type: "POST",
...
// funciÃ³n para recuperar datos del usuario a editar
    $(document).on("click", '.editar', function (event) {
        ...
        $.ajax({// configuraciÃ³n de ajax
            ...
            $('#formularioEditar input[name="id"]').val(data[0]['id']); // selecciÃ³n por id y atributo
            ...
            var line_shipping_status_id = data[0]['line_shipping_status_id'];

            $("#edit_ship_status option:selected").prop("selected");

            $('#edit_ship_status option[value="' + line_shipping_status_id + '"]').attr('selected', 'selected');
           
            if (data[0]['default_origin'] == '1') { // default_origin                
             $('#head_office_edit').prop('checked','checked');
             $('#formularioEditar input[name="hid_head_office"]').val('1');                
            } else {
              $('#head_office_edit').removeAttr('checked',false);           
              $('#formularioEditar input[name="hid_head_office"]').val('0');  
            }
