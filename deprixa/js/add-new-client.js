            ...
            "aButtons": [ // configuración de botones
                {
					...
					"mColumns": [ 0, 1, 2, 3, 4, 5, 6 ] // columnas que seran copiadas
...
// var datos = $('#add_entry').serialize();	// recuperamos todos los campos del formulario
...
$(document).on("click", '.editar', function (event) {
			...
			dataType: 'json'
					...
					$('#formularioEditar input[name="phone"]').val(data[0]['phone']);
                                        $('#formularioEditar input[name="work"]').val(data[0]['work']);
                                        $('#formularioEditar input[name="cell"]').val(data[0]['cell']);
					$('#formularioEditar input[name="email"]').val(data[0]['email']);
					$('#formularioEditar input[name="password"]').val(data[0]['password']);
					$('#formularioEditar input[name="zipcode"]').val(data[0]['zipcode']);
					$('#formularioEditar input[name="cedula"]').val(data[0]['cedula_no']);
					city_name = data[0]['city'];
					state_name = data[0]['state'];
					country_name = data[0]['country'];					
					$('#country1 option').map(function () {
